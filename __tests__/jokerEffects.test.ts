...
describe('Joker Effects Test Suite', () => {
  ...
  describe('Math Jokers', () => {
    test('Double Up - doubles candy price for one period', () => {
      ...
      const basePrice = 10;
      const result = engine.applyEffects(basePrice, 'candy_price', {
        currentPeriod: 1,
      });
    ...
    test('Time Equation - reverses one period', () => {
      ...
      const basePeriod = 5;
      const result = engine.applyEffects(basePeriod, 'period_count', {
        currentPeriod: 1,
      });
    ...
    test('Geometric Expansion - adds 10 to inventory limit persistently', () => {
      ...
      const result = engine.applyEffects(baseInventory, 'inventory_limit', {
        currentPeriod: 1,
      });
  ...
  describe('Computer Jokers', () => {
    test('tappedIn - sets hint chance to 50%', () => {
      const joker = STANDARDIZED_JOKERS.find((j) => j.id === 11)!;
      engine.addJoker(joker, 1);

      const baseChance = 0;
      const result = engine.applyEffects(baseChance, 'hint_chance', {
        currentPeriod: 1,
      });
    ...
    test('Glitch in the Matrix - activates joker duplication', () => {
      ...
      const hasEffect = engine.hasEffect('joker_duplicate', {
        currentPeriod: 1,
      });
  ...
  describe('Home Economics Jokers', () => {
    ...
    test('Pomodoro Timer - increases study time by 50%', () => {
      ...
      const baseTime = 60;
      const result = engine.applyEffects(baseTime, 'study_time', {
        currentPeriod: 1,
      });
    ...
    test('Deep Storage - adds 30 to inventory limit for one period', () => {
      ...
      const result1 = engine.applyEffects(baseInventory, 'inventory_limit', {
        currentPeriod: 1,
      });
      expect(result1).toBe(60); // 30 + 30

      // Should not work in period 2 (one-time effect expired)
      const result2 = engine.applyEffects(baseInventory, 'inventory_limit', {
        currentPeriod: 2,
      });
  ...
  describe('Economy Jokers', () => {
    test('Master of Trade - enables candy conversion', () => {
      ...
      const hasConversion = engine.hasEffect('candy_conversion', {
        currentPeriod: 1,
      });
    ...
    test('Market Manipulation - sets chosen candy to highest market price', () => {
      ...
      const newPrice = service.applyMarketManipulation(
        jokers,
        1,
        'chocolate',
        candyPrices
    ...
    test('Market Manipulation - availability check works', () => {
      const jokersWithManipulation = [
        { name: 'Market Manipulation', type: 'one-time' },
      ];
      const jokersWithout = [{ name: 'Some Other Joker', type: 'persistent' }];

      expect(service.hasMarketManipulation(jokersWithManipulation, 1)).toBe(
        true
      );
    ...
    test('The Big Short - sets chosen candy to lowest market price', () => {
      ...
      const newPrice = service.applyBigShort(
        jokers,
        1,
        'chocolate',
        candyPrices
  ...
  describe('History Jokers', () => {
    test('Roman Coin - adds $200', () => {
      ...
      const baseMoney = 100;
      const result = engine.applyEffects(baseMoney, 'money', {
        currentPeriod: 1,
      });
    ...
    test('Medieval Shield - enables event immunity for 1 period', () => {
      ...
      const hasImmunity1 = engine.hasEffect('event_immunity', {
        currentPeriod: 1,
      });
      expect(hasImmunity1).toBe(true);

      // Should expire after period 1
      const hasImmunity3 = engine.hasEffect('event_immunity', {
        currentPeriod: 3,
      });
    ...
    test('Drought Relief - adds $150 drought relief bonus', () => {
      ...
      const baseBonus = 0;
      const result = engine.applyEffects(baseBonus, 'drought_relief_bonus', {
        currentPeriod: 1,
      });
    ...
    test('Candy Vault - enables permanent stash protection', () => {
      ...
      const hasProtection1 = engine.hasEffect('stash_protection', {
        currentPeriod: 1,
      });
      expect(hasProtection1).toBe(true);

      // Should still provide protection in period 10 (persistent)
      const hasProtection10 = engine.hasEffect('stash_protection', {
        currentPeriod: 10,
      });
  ...
  describe('Logic Jokers', () => {
    test('Inductive Reasoning - adds 3 inventory each day through service', () => {
      ...
      const result1 = service.applyJokerEffects(
        20,
        'inventory_limit',
        jokers,
        4,
      ...
      const result2 = service.applyJokerEffects(
        20,
        'inventory_limit',
        jokers,
        12,
    ...
    test('Lock Pick - enables one-time event immunity', () => {
      ...
      const hasImmunity = engine.hasEffect('event_immunity', {
        currentPeriod: 1,
      });
    ...
    test('Digital Lock - doubles profits for 1 period', () => {
      ...
      // Should work in period 1
      const result1 = engine.applyEffects(basePrice, 'candy_price', {
        currentPeriod: 1,
      });
      expect(result1).toBe(200); // 100 * 2

      // Should expire after period 1
      const result3 = engine.applyEffects(basePrice, 'candy_price', {
        currentPeriod: 3,
      });
    ...
    test('Safe Deposit - enables money protection', () => {
      ...
      const hasProtection = engine.hasEffect('money_protection', {
        currentPeriod: 1,
      });
    ...
    test('Something from Nothing - enables candy generation', () => {
      ...
      const hasGeneration = engine.hasEffect('candy_generation', {
        currentPeriod: 1,
      });
  ...
  describe('Gym Jokers', () => {
    ...
    test('Embrace the Grind - adds $50 empty inventory bonus', () => {
      ...
      const baseBonus = 0;
      const result = engine.applyEffects(baseBonus, 'empty_inventory_bonus', {
        currentPeriod: 1,
      });
    ...
    test('Diamond Hand - adds $50 holding inventory bonus', () => {
      ...
      const baseBonus = 0;
      const result = engine.applyEffects(baseBonus, 'holding_inventory_bonus', {
        currentPeriod: 1,
      });
  ...
  describe('Duration and Timing Tests', () => {
    test('One-time effects expire after first period', () => {
      ...
      const result2 = engine.applyEffects(basePrice, 'candy_price', {
        currentPeriod: 2,
      });
    ...
    test('Persistent effects continue indefinitely', () => {
      ...
      const result10 = engine.applyEffects(baseInventory, 'inventory_limit', {
        currentPeriod: 10,
      });
  ...
  describe('Multiple Joker Interactions', () => {
    ...
    test('Set operations override base values', () => {
      const TappedIn = STANDARDIZED_JOKERS.find((j) => j.id === 11)!; // Set to 0.5
      const predictor = STANDARDIZED_JOKERS.find((j) => j.id === 12)!; // Set to 1.0

      engine.addJoker(TappedIn, 1);
