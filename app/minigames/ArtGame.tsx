...
const generateColorPalette = (hue: number, stageNum: number) => {
  ...
  for (let i = 0; i < totalShades; i++) {
    ...
    colors.push(
      `hsl(${hue}, ${saturation}%, ${Math.round(lightness * 10) / 10}%)`
    );
  }
...
export default function ArtGame({ onComplete }: ArtGameProps) {
  ...
  const getStageConfig = (stageNum: number) => {
    switch (stageNum) {
      case 1:
        return { gridSize: 6, pathsNeeded: 3, mistakes: 5 };
      case 2:
        return { gridSize: 7, pathsNeeded: 4, mistakes: 5 };
      case 3:
        return { gridSize: 8, pathsNeeded: 5, mistakes: 5 };
      default:
        return { gridSize: 6, pathsNeeded: 3, mistakes: 5 };
    }
  };
  ...
  const shortestPathLength = useMemo(() => {
    ...
    return (
      Math.abs(goalPosition.row - currentPosition.row) +
      Math.abs(goalPosition.col - currentPosition.col)
    );
  ...
  const generateGrid = useCallback(() => {
    ...
    for (let row = 0; row < gridSize; row++) {
      for (let col = 0; col < gridSize; col++) {
        ...
        const maxShadeInGame = Math.max(
          ...Array.from(pathShadeValues),
          maxPossibleShade
        );

        // Scale shade value to use more of the palette range
        const normalizedShade =
          grid[row][col].shadeValue / Math.max(maxShadeInGame, 1);
